...
The summary_and_plot() function takes two input variables:
...
To plot the results, set the argument inputs in the last line (the summary_and_plot function call) and run the code 
(note: this assumes the code of file 1 has been executed, creating the "data" object). The arguments are results, parameter_label, CE=False, and potion=False. 
For "results", simply put the 'data' object defined by the run_simulation call (the result of script 1). " For "parameter_label", put the name of the varying 
input parameter chosen in the simulation run: "Range","N", or "g". If cultural evolution was simulated, set "CE" to True; if potion task was simulated, set "potion" to True. 
The result is 7 network structure plots two diffusion process plots, and dataframes containing the summary statistics visualized in these plots. 
Example function calls are included in the final lines.
